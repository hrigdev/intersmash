...
/**
 * Helper class that leverages the XTF library {@link Waiter} APIs in methods that can be used to
 * control the Intersmash provisioning workflow.
 */
public class WaitersUtil {
	public static Waiter serviceEndpointsAreReady(
			OpenShift openShift, String serviceName, int numOfPods, Integer... ports) {
		return new SimpleWaiter(
				() -> {
					List<Endpoints> endpointsList = openShift.getEndpoints().stream()
					...
					if (subsets.size() < 1) {
						...
						long endpointPodsCount = subset.getAddresses().stream()
								.filter(
										endpointAddress -> endpointAddress.getTargetRef().getKind().equals("Pod"))
								.count();
	...
	public static Waiter routeIsUp(String routeURL) {
		return new SimpleWaiter(() -> Https.getCode(routeURL) != 503)
