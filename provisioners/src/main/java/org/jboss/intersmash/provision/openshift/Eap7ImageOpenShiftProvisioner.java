...
/**
 * Class deploys an EAP 7.z (i.e. Jakarta EE 8 based Wildfly) application based on {@link
 * Eap7ImageOpenShiftApplication}
 */
@Slf4j
public class Eap7ImageOpenShiftProvisioner
		implements OpenShiftProvisioner<Eap7ImageOpenShiftApplication> {
	...
	public void waitForReplicas(int replicas) {
		OpenShiftWaiters.get(openShift, ffCheck)
				.areExactlyNPodsReady(replicas, application.getName())
				.level(Level.DEBUG)
				.waitFor();
	...
	private ApplicationBuilder getAppBuilder() {
		...
		if (BinarySource.class.isAssignableFrom(buildInput.getClass())) {
			...
			// *** WAR, JAR, EAR ***
			// Binary build that expect a file on path that shall be uploaded as {@code ROOT.(suffix)}.
			// {@code suffix} is war, jar,...
	...
	private void deployImage() {
		...
		appBuilder
				...
				.setInitialDelay(60)
				.createExecProbe(
						"/bin/bash", "-c", String.format("/opt/%s/bin/livenessProbe.sh", serverHomeDirectory));
		...
		appBuilder
				...
				.addReadinessProbe()
				.createExecProbe(
						"/bin/bash", "-c", String.format("/opt/%s/bin/readinessProbe.sh", serverHomeDirectory));
		...
		if (application.getPingServiceName() != null) {
			...
			appBuilder
					.service(application.getPingServiceName())
		...
		if (!application.getCliScript().isEmpty()) {
			...
			appBuilder
					...
					.container()
					.addVolumeMount(
							"jboss-cli", String.format("/opt/%s/extensions", serverHomeDirectory), false);
		}
		...
		if (application.getEnvVars().stream()
