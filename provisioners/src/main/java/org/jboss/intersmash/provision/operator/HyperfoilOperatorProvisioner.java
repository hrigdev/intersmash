...
@Slf4j
public abstract class HyperfoilOperatorProvisioner<C extends NamespacedKubernetesClient>
		extends OperatorProvisioner<HyperfoilOperatorApplication, C>
		implements Provisioner<HyperfoilOperatorApplication>, HasPods {
	...
	public Pod getControllerPod() {
		return getPods().stream()
				.filter(
						p -> p.getMetadata()
								.getName()
								.equals(String.format("%s-controller", getApplication().getName()))
	...
	public List<Pod> getAgentPods() {
		return getPods().stream()
				.filter(
						p -> p.getMetadata().getName().startsWith("agent-")
								&& "agent".equals(p.getMetadata().getLabels().get("role")))
				.collect(Collectors.toList());
	}

	/**
	 * This method checks if the Operator's POD is actually running; It's been tailored on the
	 * community-operators Cluster Service version format which is missing label <code>
	 * spec.install.spec.deployments.spec.template.metadata.labels."app.kubernetes.io/name"</code>
	 * which is used in @see OperatorProvisioner#waitForOperatorPod() (see
	...
	protected void waitForOperatorPod() {
		...
		for (String spec : operatorSpecs) {
			...
			new SimpleWaiter(
					() -> getPods().stream()
							.filter(
									pod -> (pod.getMetadata().getName().startsWith(operatorSpec[1])
	...
	public void deploy() {
		...
		new SimpleWaiter(() -> hyperfoil().get().getStatus() != null)
				...
				.waitFor();
		new SimpleWaiter(
				() -> getControllerPod() != null
						&& getControllerPod().getStatus().getConditions().stream()
								.anyMatch(c -> "Ready".equals(c.getType()) && "True".equals(c.getStatus())))
				.failFast(ffCheck)
				.reason("Wait for expected number of replicas to be active.")
				.level(Level.DEBUG)
				.waitFor();
		new SimpleWaiter(this::routeIsReadyAndUp)
	...
	private boolean routeIsReadyAndUp() {
		try {
			...
			// doesn't seem to support, i.e. if the above fails, then the waiter blows up, no waiting at
			// all
			log.debug(
	...
	public void scale(int replicas, boolean wait) {
		throw new UnsupportedOperationException(
				"Scaling is not implemented by Hyperfoil operator based provisioning");
	}
	...
	/**
	 * Get a client capable of working with {@link HyperfoilOperatorProvisioner#HYPERFOIL_CRD_NAME}
	 * custom resource.
	 *
	 * @return client for operations with {@link HyperfoilOperatorProvisioner#HYPERFOIL_CRD_NAME}
	 *     custom resource
	 */
	public NonNamespaceOperation<Hyperfoil, HyperfoilList, Resource<Hyperfoil>> hyperfoilClient() {
		if (HYPERFOIL_CUSTOM_RESOURCE_CLIENT == null) {
			CustomResourceDefinition crd = customResourceDefinitionsClient().withName(HYPERFOIL_CRD_NAME).get();
			if (crd == null) {
	...
	/**
	 * Get a reference to Hyperfoil object. Use get() to get the actual object, or null in case it
	 * does not exist on tested cluster.
	 *
	 * @return A concrete {@link Resource} instance representing the {@link Hyperfoil} resource
	 *     definition
	 */
