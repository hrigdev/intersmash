...
public class KeycloakOpenShiftOperatorProvisioner
	...
	public void subscribe() {
		if (Strings.isNullOrEmpty(IntersmashConfig.keycloakImageURL())) {
			...
			subscribe(
					...
					Map.of(
							// Custom Keycloak image to be used: overrides the Keycloak image at the operator
							// level: all
							// Keycloak instances will be spun out of this image
							// e.g. OPERATOR_KEYCLOAK_IMAGE=quay.io/keycloak/keycloak:21.1.1 -->
	...
	@Override
	public URL getURL() {
		String host = OpenShiftProvisioner.openShift.routes().list().getItems().stream()
				.filter(
						route -> route
								.getMetadata()
								.getName()
								.startsWith(keycloak().get().getMetadata().getName())
								&& route.getMetadata().getLabels().entrySet().stream()
										.filter(
												label -> label
														.getKey()
														.equalsIgnoreCase("app.kubernetes.io/instance")
														&& label
																.getValue()
																.equalsIgnoreCase(
																		keycloak()
				...
				.orElseThrow(
						() -> new RuntimeException(
								String.format(
										"No route for Keycloak %s!", keycloak().get().getMetadata().getName())))
				.getSpec()
				.getHost();
		try {
			...
			throw new RuntimeException(
					String.format("Keycloak operator External URL \"%s\" is malformed.", host), e);
		}
	}
	...
	public HasMetadataOperationsImpl<Keycloak, KeycloakOperatorKeycloakList> keycloaksCustomResourcesClient(
		...
		return OpenShifts.master()
				.newHasMetadataOperation(crdc, Keycloak.class, KeycloakOperatorKeycloakList.class);
	}
	...
	public HasMetadataOperationsImpl<KeycloakRealmImport, KeycloakOperatorRealmImportList> keycloakRealmImportsCustomResourcesClient(
		...
		return OpenShifts.master()
				.newHasMetadataOperation(
						crdc, KeycloakRealmImport.class, KeycloakOperatorRealmImportList.class);
	}
}
