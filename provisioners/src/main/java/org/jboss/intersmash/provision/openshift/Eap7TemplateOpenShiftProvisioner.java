...
public class Eap7TemplateOpenShiftProvisioner
	...
	private void deployTemplate() {
		...
		// values explicitly, based on the previously deployed resources, in case those were not
		// provided.
		if (!params.containsKey("EAP_IMAGE_NAME")) {
			...
			if (!deployedBuilderImageNameSearch.isPresent()) {
				throw new IllegalStateException(
						String.format(
								"The expected EAP 7 builder image stream was not found in the %s namespace",
		...
		if (!params.containsKey("EAP_RUNTIME_IMAGE_NAME")) {
			...
			if (!deployedRuntimeImageName.isPresent()) {
				throw new IllegalStateException(
						String.format(
								"The expected EAP 7 runtime image stream was not found in the %s namespace",
								openShift.getNamespace()));
			}
	...
	private void postDeploy(WildflyOpenShiftApplication eapApplication) {
		...
		if (eapApplication.getCliScript() != null && !eapApplication.getCliScript().isEmpty()) {
			...
			ConfigMap cfMap = new ConfigMapBuilder()
					.withNewMetadata()
					.withName("jboss-cli")
					.withLabels(Collections.singletonMap(APP_LABEL_KEY, eapApplication.getName()))
					.endMetadata()
					.addToData("postconfigure.sh", postconfigure)
					.addToData("configure.cli", String.join("\n", eapApplication.getCliScript()))
					.build();
			openShift.createConfigMap(cfMap);
			// TODO make it JAVA
			// https://access.redhat.com/documentation/en-us/red_hat_jboss_enterprise_application_platform/7.3/html-single/getting_started_with_jboss_eap_for_openshift_container_platform/index#custom_scripts

			String output = OpenShifts.masterBinary()
					.execute(
							"set",
							"volume",
							"dc/" + eapApplication.getName(),
							"--add",
							"--name=jboss-cli",
							"-m",
							"/opt/eap/extensions",
