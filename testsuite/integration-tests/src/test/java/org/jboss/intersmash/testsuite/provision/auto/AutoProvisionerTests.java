...
/** Smoke test cases that cover usage of {@link OpenShiftAutoProvisioner} */
@Slf4j
@OpenShiftTest
public class AutoProvisionerTests {

	/**
	 * Tests that expected custom exception is thrown when something goes bad in the {@link
	 * OpenShiftApplication} which is implementing the Auto Provisioning workflow.
	 */
	@Test
	public void testCustomExceptionsAreThrownForWorkflowErrors() {
		...
		AutoProvisioningManagedException ex = Assertions.assertThrows(
				AutoProvisioningManagedException.class, () -> provisioner.preDeploy());
		...
		ex = Assertions.assertThrows(
				AutoProvisioningManagedException.class, () -> provisioner.scale(0, false));
		logWrappedUpError(ex);
		ex = Assertions.assertThrows(
				AutoProvisioningManagedException.class, () -> provisioner.undeploy());
		logWrappedUpError(ex);
		ex = Assertions.assertThrows(
				AutoProvisioningManagedException.class, () -> provisioner.postUndeploy());
		logWrappedUpError(ex);
	}
	...
	private void logWrappedUpError(AutoProvisioningManagedException ex) {
		log.info(
				String.format(
						"An error (%s) occurred during the auto provisioning workflow execution: %s",
